...
const page = ({
  ...
  useEffect(()=>{
    const fetchSingleItenary = async()=>{
      try {
        const res = await fetch('https://dashboard.geranosgetaways.com/wp-json/wp/v2/itineraries?slug=lorem-ipsum')
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="max-w-7xl mx-auto px-4 mt-12 flex flex-col lg:flex-row gap-12">
        ...
        <div className="flex-1">
          <div className="text-sm text-blue-400 mb-2">{destination}</div>
          <h1 className="text-3xl font-bold mb-4">{title}</h1>
          <p className="text-gray-600 mb-6">
            Crystal blue waters, huge mountains, golden beaches and ancient ruins, {destination} offers the most
            fulfilling experiences in the world.
          </p>
          ...
          <div className="mb-8">
            ...
            <p className="text-gray-800 font-semibold text-xl">
              ₹{price} <span className="text-sm font-light">per person</span>
            </p>
          </div>
        ...
        <div className="w-full lg:w-1/3 bg-white shadow-lg rounded-xl p-6">
          ...
          <form className="space-y-3">
            ...
            <div className="text-sm text-gray-500">
              Starting from <span className="font-semibold text-black">₹{price}</span> per person
            </div>
            <button
              type="submit"
