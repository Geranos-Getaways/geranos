  ...
  return (
    <div className="flex flex-col md:flex-row max-w-6xl m-auto py-8 px-8 sm:px-8 gap-8 border-b-2">
      <div className="flex flex-col gap-4 align-middle justify-center text-center md:text-left">
        ...
        <p className="text-slate-400  text-sm sm:text-xl font-UrbanistLight tracking-wide">
          {d.getDate() + '/' + d.getMonth() + '/' + d.getFullYear()}
        </p>
        <div
          className="text-sm sm:text-xl font-UrbanistLight tracking-wide"
          dangerouslySetInnerHTML={{ __html: excerpt }}
        />
        <Link href={`/blog/${slug}`}>
          <Button className="bg-[#2F6BEB] text-justify text-sm sm:text-base font-UrbanistLight tracking-wide">
            Read Blog
          </Button>
        </Link>
      </div>
      ...
      <div className="flex flex-col order-[-1] md:order-1 gap-4 align-middle justify-center md:min-w-[280px] xl:min-w-[320px]">
        <Image
          ...
          height="400"
          alt="blog thumbnail"
          className="m-auto rounded-xl mx-h-[200px] sm:max-h-full"
        />
      </div>
    </div>
  );
