...
const Layout = ({ children, params }: LayoutProps) => {
  ...
  useEffect(() => {
    const fetchDestination = async () => {
      const res = await fetch(`https://dashboard.geranosgetaways.com/wp-json/wp/v2/destinations?slug=${state}`);
  ...
  <Image 
    width={1920}
    height={500}
    src={featuredImage?.guid?.rendered || `/global/${state}.webp`} 
    alt={`${state} Hero Section`}
    className="w-full h-full object-cover object-center" 
  />

  {/* Overlay content */}
  <div className="absolute inset-0 flex flex-col items-center justify-center text-white text-center" style={{ backgroundColor: '#00000075' }}>
    <h1 className="text-4xl md:text-8xl font-bold drop-shadow-lg capitalize font-bropella">{state}</h1>
    <p className="text-lg md:text-2xl mt-2 drop-shadow-md">{destination?.acf?.sub_heading}</p>
  </div>

  {/* Nav bar positioned above bottom */}
  <div className="absolute bottom-6 left-1/2 transform -translate-x-1/2 z-30">
  <ul className="flex gap-4 text-lg bg-slate-200 px-6 py-2 rounded-full shadow-md" style={{ backgroundColor: '#ffffffa3' }}>
    <li>
      <Link
